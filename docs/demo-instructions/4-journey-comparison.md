...
`comparisons/digital-vs-branch.md` — one markdown table, one row per step across the union of both journeys (14 rows: 11 shared steps + 3 branch-only steps), with columns roughly: step name, present in digital?, present in branch?, shared vs. channel-specific, and a plain-language rule difference note.
